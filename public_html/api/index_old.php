<?php
/**
 * Point d'entrée API REST - Self-contained
...
// ========================================
// Database Connection (Direct - No Bootstrap)
// ========================================
...
class SimpleRouter {
    ...
    public function dispatch() {
        ...
        foreach ($this->routes as $route => $handler) {
            ...
            $routePath = preg_replace('/\\\\:([a-z_]+)/', '(?P<$1>[^/]+)', $routePath);
        ...
        return json_encode(['error' => 'Not found'], JSON_UNESCAPED_UNICODE);
    }
}
...
try {
     ...
     */
    $router->get('/debug/tables', function() use ($db) {
        $tables = [
            'ecritures' => 0
        ];
     ...
     */
    $router->get('/annee/:exercice/exists', function($exercice) use ($db) {
        try {
            $stmt = $db->prepare("SELECT COUNT(*) as cnt FROM ecritures WHERE exercice = ?");
            $stmt->execute([(int) $exercice]);
